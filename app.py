...
def signal_on_last_bar(cs):
    # EMA50 mean-reversion V3.1
    # SHORT: price >= 1.50% above EMA50 AND current candle is red.
    # LONG:  price <= 1.50% below EMA50 AND current candle is green.
    # No EMA9/VWAP/RSI/volume filters.
    if len(cs)<55:
        return None
    ...
    if last['close'] >= upper and last['close'] < last['open']:
        return 'SHORT'
    if last['close'] <= lower and last['close'] > last['open']:
        return 'LONG'
    return None
...
def run_backtest_30d():
    ...
    try:
        set_state('bt_status','running'); set_state('bt_progress','0'); set_state('bt_message','Starting EMA50 Mean-Reversion V3.1 backtest...'); set_state('bt_result','')
