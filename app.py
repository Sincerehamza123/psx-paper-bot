...
def signal_on_last_bar(cs):
    ...
    return e9[-1]>e20[-1]>e50[-1] and last['close']>vwap and last['close']>e9[-1] and RSI_MIN<=rs[-1]<=RSI_MAX and avg>0 and last['volume']>=VOLUME_MULTIPLIER*avg and body>=MIN_BODY_PCT and last['close']>last['open']
...
def run_backtest_30d():
    ...
    try:
        set_state('bt_status','running'); set_state('bt_progress','0'); set_state('bt_message','Starting Strategy V2 backtest...'); set_state('bt_result','')
        ...
        set_state('bt_result',json.dumps(result)); set_state('bt_progress','100'); set_state('bt_message','Strategy V2 30-day backtest completed.'); set_state('bt_status','completed')
