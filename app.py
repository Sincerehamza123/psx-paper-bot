...
STATE_VERSION="bybit-futures-usd-100-v4"
POSITION_PCT=0.10
...
def fetch_klines(symbol, limit=120):
    # Bybit V5 public USDT perpetual (linear) 1-minute klines.
    ...
    req = Request(
        BYBIT_KLINES_URL + "?" + params,
        headers={"User-Agent": "Mozilla/5.0 crypto-paper-bot"}
    )
    ...
    if int(payload.get("retCode", -1)) != 0:
        raise RuntimeError(
            "Bybit API error: " + str(payload.get("retMsg", "unknown error"))
        )
    ...
    candles = []
    # Bybit returns newest first, so reverse to oldest -> newest.
    for k in reversed(raw):
        open_time = int(k[0])
        # Ignore the still-forming current 1-minute candle.
        if open_time >= current_minute_start:
            continue
        candles.append({
            "open_time_ms": open_time,
            "close_time_ms": open_time + 60000 - 1,
            "open": float(k[1]),
            "high": float(k[2]),
            "low": float(k[3]),
            "close": float(k[4]),
